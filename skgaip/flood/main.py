...
from ealstm.papercode.datautils import add_camels_attributes, load_attributes, rescale_features
...
def get_args() -> Dict:
    ...
    parser.add_argument(
        "--cache_data", type=bool, default=False, help="If True, loads all data into memory"
    )
    parser.add_argument(
        "--num_workers", type=int, default=12, help="Number of parallel threads for data loading"
    )
    parser.add_argument(
        "--no_static", type=bool, default=False, help="If True, trains LSTM without static features"
    )
    parser.add_argument(
        "--concat_static",
        type=bool,
...
def eval_robustness(user_cfg: Dict):
    ...
    out_file = (
        Path(__file__).absolute().parent / f'results/{user_cfg["run_dir"].name}_model_robustness.p'
    )
