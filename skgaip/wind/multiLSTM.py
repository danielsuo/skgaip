...
class multiLSTM:
    ...
    def drawGraphStation(self, station, visualise=1, ax=None):
        ...
        print(
            "station %s : MAE = %7.7s   RMSE = %7.7s    nrmse_maxMin = %7.7s   nrmse_mean = %7.7s"
    ...
    def run(self):
        ...
        pickle.dump({"models": self.lstmModels}, open("models.pkl", "wb"))
        # testing
        print("...... TESTING  ...")
        self.test()
        self.test_train()
        pickle.dump(
            {"pred": self.predicted, "pred_train": self.predicted_train}, open("output.pkl", "wb")
        )
